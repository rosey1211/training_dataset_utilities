...
int main(int argc, char* argv[])
{
    ...
    for (const auto& pngPath : pngPaths) {
        ...
        for (int v = 0; v < cfg.num_augmentations && !aborted; ++v) {
            // Every version applies BOTH a random shift AND a random warp.
            // Order: shift first, then warp on the shifted result.
            // Each parameter is drawn independently per version.
            double shift     = shiftDist(rng);
            double A         = warpADist(rng);
            double k         = warpKDist(rng);
            double warpSign  = (signDist(rng) == 0) ? 1.0 : -1.0;

            // Step 1 — horizontal shift
            cv::Mat shifted  = applyShift(src, shift, cfg.inpaint_radius);

            // Intermediate label after shift (use a dummy filename; replaced below)
            LabelRow shiftedLabel = transformLabel(srcLabel, "", headerCols, shift);

            // Step 2 — perspective warp on the shifted image
            cv::Mat augImg = applyPerspectiveWarp(
                shifted, lowestFrac, highestFrac,
                A, k, cfg.warp_n, warpSign, cfg.inpaint_radius);

            std::string augFname = stem + "_sw" + std::to_string(v) + ".png";

            // Chain label: warp applied on top of the shifted label
            shiftedLabel.filename = augFname;
            LabelRow augLabel = transformLabel_warp(
                shiftedLabel, augFname, headerCols,
                lowestFrac, highestFrac,
                A, k, cfg.warp_n, warpSign);

            // Description for console + visualisation
            std::ostringstream d;
            d << "SHIFT" << (shift >= 0 ? "+" : "")
